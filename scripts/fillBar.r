...
print_usage <- function(spec=NULL){
	...
	cat("
Usage example: 
1) Rscript fillBar.r --infile in_fillBar.data --outfile out_fillBar.png \\
	--x.col 1 --group.col 2 --y.col 3 --x.lab \"x lab\" --group.lab \"group lab\" \\
	--y.lab \"y lab\" --title.lab \"title lab\" --legend.col 2
2) Rscript fillBar.r --infile in_fillBar.data --outfile out_fillBar.png \\
	--x.col 1 --group.col 2 --y.col 3 --x.lab \"x lab\" --group.lab \"group lab\" \\
	--y.lab \"y lab\" --title.lab \"title lab\" --height 3000 --width 4000
3) Rscript fillBar.r --infile in_fillBar.data --outfile out_fillBar.png \\
	--x.col 1 --group.col 2 --y.col 3 --x.lab \"x lab\" --group.lab \"group lab\" \\
...
--x.angle	-xa	integer		the x text angle [default:0]
...
#-----------------------------------------------------------------
# mian plot
#p <- qplot(x, y, data=df, geom="bar", stat="identity", fill=group)
p <- ggplot(df, aes(x=x, y=y, fill=group))
#p <- p + geom_bar(aes(width=1), colour="black", stat="identity", position="fill")
p <- p + geom_bar(colour="black", stat="identity", position="fill")
#p <- qplot(x, y, data=df, geom="bar", stat="identity", fill=group)
...
p <- p + theme(title = element_text(face="bold", size=opt$lab.size), 
	axis.text = element_text(face="bold", size=opt$axis.size,angle=opt$x.angle))
